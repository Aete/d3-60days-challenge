...
export default function ScatterChartOne(element, countryFilter) {
  ...
  const yScale = d3.scaleLinear().domain([0, 1430000000]).range([height, 0]);
  ...
  this.update = (year, countryList) => {
    ...
    function brushed({ selection }) {
      ...
      if (selection) {
        ...
        values = dot.filter((d) => x0 <= xScale(d.gdp_cap) && xScale(d.gdp_cap) < x1 && y0 <= yScale(d.pop) && yScale(d.pop) < y1).data();
        console.log(values);
