...
export default function ScatterChart(element) {
  ...
  let xScale = d3.scaleLinear().domain([0, 100000]).range([0, width]);
  const yScale = d3.scaleLinear().domain([0, 100]).range([height, 0]);
  const rScale = d3.scaleSqrt().domain([140000, 1430000000]).range([2, 20]);

  let xAxisFunction = d3.axisBottom(xScale).tickSizeOuter(0).tickFormat(d3.format('.2s'));
  ...
  this.redraw = () => {
    const currentWidth = element.getBoundingClientRect().width - margin.right - margin.left;
    svg
      .transition()
      .duration(500)
      .attr('width', currentWidth + margin.right + margin.left);

    let xScale = d3.scaleLinear().domain([0, 100000]).range([0, currentWidth]);
    let xAxisFunction = d3.axisBottom(xScale).tickSizeOuter(0).tickFormat(d3.format('.2s'));

    xAxisTitle.attr('x', currentWidth);

    xAxis.call(xAxisFunction);

    const dots = dotContainer.selectAll('circle');
